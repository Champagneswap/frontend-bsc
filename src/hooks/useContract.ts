...
import {
  ...
  getErc721Contract,
  getChamVaultContract,
  getIfoPoolContract,
  getPredictionsContract,
  getChainlinkOracleContract,
  getLotteryV2Contract,
  getBunnySpecialChamVaultContract,
...
export const useCham = (withSignerIfPossible = true) => {
  const { account, library } = useActiveWeb3React()
  return useMemo(
    () => getChamContract(withSignerIfPossible ? getProviderOrSigner(library, account) : null),
...
export const useVaultPoolContract = (vaultKey: VaultKey): ChamVault | IfoPool => {
  const { library } = useActiveWeb3React()
  return useMemo(() => {
    return vaultKey === VaultKey.ChamVault
      ? getChamVaultContract(library.getSigner())
      : getIfoPoolContract(library.getSigner())
  }, [library, vaultKey])
}

export const useChamVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getChamVaultContract(library.getSigner()), [library])
}
...
export const useSpecialBunnyChamVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getBunnySpecialChamVaultContract(library.getSigner()), [library])
}
...
export function usePanPairContract(pairAddress?: string, withSignerIfPossible?: boolean): IChampagnePair | null {
  return useContract(pairAddress, IChampagneABI, withSignerIfPossible)
}
