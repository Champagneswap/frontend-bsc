import { ChainId, Currency, currencyEquals, JSBI, Price } from '@champagneswap/sdk'
...
export const useChamBusdPrice = (): Price | undefined => {
  const chamBusdPrice = useBUSDPrice(tokens.cham)
  return chamBusdPrice
}
...
export const useBUSDChamAmount = (amount: number): number | undefined => {
  const chamBusdPrice = useChamBusdPrice()
  if (chamBusdPrice) {
    return multiplyPriceByAmount(chamBusdPrice, amount)
  }
  return undefined
}
