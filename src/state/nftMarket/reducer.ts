...
>('nft/fetchNftsFromCollections', async ({ collectionAddress, page, size }) => {
  try {
    if (collectionAddress === champagneBunniesAddress) {
  ...
  async ({ bunnyId, existingTokensWithBunnyId, allExistingPBTokenIds, existingMetadata, orderDirection }) => {
    try {
      ...
      const [updatedNfts, updatedNftsMarket] = await Promise.all([
        getNftsFromCollectionApi(champagneBunniesAddress),
        getMarketDataForTokenIds(champagneBunniesAddress, allExistingPBTokenIds),
      ])
      ...
      if (!existingMetadata) {
        newNfts = await getNftsFromCollectionApi(champagneBunniesAddress)
      }
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchNewPBAndUpdateExisting.fulfilled, (state, action) => {
      if (action.payload.length > 0) {
        state.data.nfts[champagneBunniesAddress] = action.payload
      }
