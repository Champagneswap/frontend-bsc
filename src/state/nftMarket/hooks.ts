...
export const useFetchByBunnyIdAndUpdate = (bunnyId: string) => {
  ...
  const allPancakeBunnies = useNftsFromCollection(champagneBunniesAddress)
...
export const useGetAllBunniesByBunnyId = (bunnyId: string) => {
  const nfts: NftToken[] = useSelector((state: State) => state.nftMarket.data.nfts[champagneBunniesAddress])
...
export const useApprovalNfts = (nftsInWallet: NftToken[]) => {
  ...
  const approvedTokenIds = Array.isArray(data)
    ? data
        .flat()
        .reduce(
          (acc, address, index) => ({ ...acc, [nftsInWallet[index].tokenId]: getChampagneProfileAddress() === address }),
          {},
        )
    : null
