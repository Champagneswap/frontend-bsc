...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({
  ...
  const isChamBalanceInsufficient = chamBalance.lt(pricePerTicket)
  const maxBuyTickets = Math.min(chamBalance.div(pricePerTicket).toNumber(), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px">{formatBigNumber(pricePerTicket, 0)} CHAM</Text>
            </Flex>
            <Flex mb="8px" justifyContent="space-between">
              <Text font-size="14px" color="textSubtle">
                {t('Your CHAM Balance')}
              </Text>
              <Text font-size="14px" color={isChamBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigNumber(chamBalance, 3)} CHAM
              </Text>
            </Flex>
            <Flex
              mb="8px"
            ...
            <Flex mb="25px" justifyContent="space-between">
              ...
              <Text font-size="14px" bold>
                {formatBigNumber(totalCost, 0)} CHAM
              </Text>
            </Flex>
          </Box>
          <Flex
          ...
          <Box px="16px">
            <Button
              disabled={isChamBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isChamBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
