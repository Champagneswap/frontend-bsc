...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumChamRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumChamRequired = fetchStatus === FetchStatus.Fetched && chamBalance.gte(REGISTER_COST)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumChamRequired={minimumChamRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumChamRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% CHAM is required', { num: formatUnits(REGISTER_COST) })}
        </Text>
      )}
    </>
  )
