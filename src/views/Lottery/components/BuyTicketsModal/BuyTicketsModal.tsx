...
import { useCham, useLotteryV2Contract } from 'hooks/useContract'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const stringifiedUserCham = userCham.toJSON()
  const memoisedUserCham = useMemo(() => new BigNumber(stringifiedUserCham), [stringifiedUserCham])

  const chamPriceBusd = usePriceChamBusd()
  const dispatch = useAppDispatch()
  const hasFetchedBalance = fetchStatus === FetchStatus.Fetched
  const userChamDisplayBalance = getFullDisplayBalance(userCham, 18, 3)
  ...
  const validateInput = useCallback(
    ...
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userCham],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCham.div(priceTicketInCham)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max CHAM balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInCham.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInCham, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCham) return t('Insufficient CHAM balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        isWarning={account && (userNotEnoughCham || maxTicketPurchaseExceeded)}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          chamPriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInCham.times(new BigNumber(ticketsToBuy))) : '0.00'} CHAM`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {account && (userNotEnoughCham || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCham && getFullDisplayBalance(priceTicketInCham.times(ticketsToBuy || 0))} CHAM
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
