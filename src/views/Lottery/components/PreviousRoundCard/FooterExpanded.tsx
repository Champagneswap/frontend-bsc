...
import { Flex, Skeleton, Heading, Box, Text } from '@champagneswap/uikit'
...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInCham } = lotteryNodeData
    prizeInBusd = amountCollectedInCham.times(chamPriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" CHAM"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInCham)}
            decimals={0}
          />
        )}
      </>
