...
import { Flex, Skeleton, Text } from '@champagneswap/uikit'
...
const RewardBracketDetail: React.FC<RewardBracketDetailProps> = ({
  ...
  return (
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || chamAmount.isNaN() ? (
          <Skeleton my="4px" mr="10px" height={20} width={110} />
        ) : (
          <Balance fontSize="20px" bold unit=" CHAM" value={getBalanceNumber(chamAmount)} decimals={0} />
        )}
        {isLoading || chamAmount.isNaN() ? (
          <>
          ...
          <Balance
            ...
            color="textSubtle"
            prefix="~$"
            value={getBalanceNumber(chamAmount.times(chamPriceBusd))}
            decimals={0}
          />
        )}
        {isHistoricRound && chamAmount && (
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(chamAmount.div(parseInt(numberWinners, 10)), 18, 2)} CHAM {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
