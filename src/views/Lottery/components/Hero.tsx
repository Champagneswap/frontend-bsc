...
import { Box, Flex, Heading, Skeleton } from '@champagneswap/uikit'
...
const Hero = () => {
  ...
  const {
    currentRound: { amountCollectedInCham, status },
    isTransitioning,
  } = useLottery()

  const chamPriceBusd = usePriceChamBusd()
  const prizeInBusd = amountCollectedInCham.times(chamPriceBusd)
