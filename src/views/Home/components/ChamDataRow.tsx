import { Flex, Heading, Skeleton, Text } from '@champagneswap/uikit'
...
const ChamDataRow = () => {
  ...
  useSlowRefreshEffect(() => {
    const fetchTokenData = async () => {
      const totalSupplyCall = { address: tokens.cham.address, name: 'totalSupply' }
      const burnedTokenCall = {
        address: tokens.cham.address,
        name: 'balanceOf',
        params: ['0x000000000000000000000000000000000000dEaD'],
      }
      const tokenDataResultRaw = await multicallv2(chamAbi, [totalSupplyCall, burnedTokenCall], {
        requireSuccess: false,
      })
      const [totalSupply, burned] = tokenDataResultRaw.flat()
      setChamSupply(totalSupply && burned ? +formatBigNumber(totalSupply.sub(burned)) : 0)
