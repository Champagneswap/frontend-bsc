...
import { Heading, Flex, Image, Text } from '@champagneswap/uikit'
...
import ChamVaultCard from './components/ChamVaultCard'
import PoolTabButtons from './components/PoolTabButtons'
import BountyCard from './components/BountyCard'
import HelpButton from './components/HelpButton'
import PoolsTable from './components/PoolsTable/PoolsTable'
import { getChamVaultEarnings } from './helpers'
        ...
        return orderBy(
          ...
          (pool: DeserializedPool) => {
            ...
            return pool.vaultKey
              ? getChamVaultEarnings(
                  account,
                  vaultPools[pool.vaultKey].userData.chamAtLastUserAction,
      ...
      case 'totalStaked':
        return orderBy(
          ...
          (pool: DeserializedPool) => {
            ...
            if (pool.vaultKey) {
              if (pool.stakingTokenPrice && vaultPools[pool.vaultKey].totalChamInVault.isFinite()) {
                totalStaked =
                  +formatUnits(
                    EthersBigNumber.from(vaultPools[pool.vaultKey].totalChamInVault.toString()),
                    pool.stakingToken.decimals,
                  ) * pool.stakingTokenPrice
              }
            } else if (pool.sousId === 0) {
              if (pool.totalStaked?.isFinite() && pool.stakingTokenPrice && chamInVaults.isFinite()) {
                const manualChamTotalMinusAutoVault = EthersBigNumber.from(pool.totalStaked.toString()).sub(
                  chamInVaults.toString(),
                )
                totalStaked =
                  +formatUnits(manualChamTotalMinusAutoVault, pool.stakingToken.decimals) * pool.stakingTokenPrice
              }
  ...
  const cardLayout = (
    <CardLayout>
      {chosenPools.map((pool) =>
        pool.vaultKey ? (
          <ChamVaultCard key={pool.vaultKey} pool={pool} showStakedOnly={stakedOnly} />
        ) : (
