...
import RecentChamProfitRow from './RecentChamProfitRow'

const StyledCardBody = styled(CardBody)<{ isLoading: boolean }>`
  min-height: ${({ isLoading }) => (isLoading ? '0' : '254px')};
`

interface ChamVaultProps extends CardProps {
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    hasEndBlockOver ? (
      <>
        ...
        <LinkExternal href="https://twitter.com/champagnedefi">
      ...
      <>
        <Text>
          {t(
            'The start block of the current calculation period. Your average IFO CHAM Pool staking balance is calculated throughout this period.',
          )}
        </Text>
        <LinkExternal href="https://medium.com/champagneswap/">
...
const ChamVaultCard: React.FC<ChamVaultProps> = ({ pool, showStakedOnly, defaultFooterExpanded, ...props }) => {
  const { t } = useTranslation()
    ...
    <StyledCard isActive {...props}>
      ...
      <StyledCardBody isLoading={isLoading}>
        ...
        <FlexGap mt="16px" gap="24px" flexDirection={accountHasSharesStaked ? 'column-reverse' : 'column'}>
          <Box>
            <Box mt="24px">
              <RecentChamProfitRow vaultKey={pool.vaultKey} />
            </Box>
            <Box mt="8px">
