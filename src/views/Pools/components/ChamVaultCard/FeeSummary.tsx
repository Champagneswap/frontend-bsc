...
import { Text, Flex, useTooltip, TooltipText } from '@champagneswap/uikit'
...
const FeeSummary: React.FC<FeeSummaryProps> = ({ stakingTokenSymbol, stakeAmount, vaultKey }) => {
  ...
  const feeInCham = (parseFloat(stakeAmount) * (feeAsDecimal / 100)).toFixed(4)
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'Only applies within %num% days of staking. Unstaking after %num% days will not include a fee. Timer resets every time you stake new CHAM in the pool.',
          {
  ...
  return (
    <>
      <Flex mt="24px" alignItems="center" justifyContent="space-between">
        ...
        <Text fontSize="14px">
          {stakeAmount && hasFeeToPay ? feeInCham : '-'} {stakingTokenSymbol}
        </Text>
      </Flex>
