...
import { fetchChamVaultUserData } from 'state/pools'
...
import { convertChamToShares, convertSharesToCham } from '../../helpers'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(chamPriceBusd)
  const formattedUsdValueStaked = chamPriceBusd.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    const shareStakeToWithdraw = convertChamToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 CHAM or less
  ...
  if (showRoiCalculator) {
    return (
      <RoiCalculatorModal
        ...
        stakingTokenBalance={chamAsBigNumber.plus(stakingMax)}
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={chamPriceBusd.gt(0) && `~${formattedUsdValueStaked || 0} USD`}
