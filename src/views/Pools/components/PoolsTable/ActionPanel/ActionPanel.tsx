...
import { convertSharesToCham, getPoolBlockInfo } from 'views/Pools/helpers'
...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const poolStakingTokenBalance = vaultKey
    ? chamAsBigNumber.plus(stakingTokenBalance)
        ...
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {vaultKey
              ...
              : `${t('Earn')} CHAM ${t('Stake').toLocaleLowerCase()} CHAM`}
          </Text>
        )}
        {pool.vaultKey ? (
