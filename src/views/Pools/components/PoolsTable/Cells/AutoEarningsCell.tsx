...
import { Skeleton, Text, useTooltip, HelpIcon, Flex, Box, useMatchBreakpoints } from '@champagneswap/uikit'
import { DeserializedPool } from 'state/types'
import Balance from 'components/Balance'
import { useVaultPoolByKey } from 'state/pools/hooks'
import { useTranslation } from 'contexts/Localization'
import { convertSharesToCham, getChamVaultEarnings } from 'views/Pools/helpers'
...
const getAutoEarningsInterestBreakdown = (
  ...
  return getInterestBreakdown({
    principalInUSD: getBalanceNumber(chamAsBigNumber.times(stakingTokenPrice)),
    apr: rawApr,
    earningTokenPrice,
...
const AutoEarningsCell: React.FC<AutoEarningsCellProps> = ({ pool, account }) => {
  ...
  const {
    userData: { isLoading: userDataLoading, chamAtLastUserAction, userShares, lastUserActionTime },
  ...
  const labelText = t('Recent CHAM profit')
  const earningTokenBalance = autoChamToDisplay
  ...
  const earnedChamPerHour = hourDiffSinceLastAction ? earningTokenBalance / hourDiffSinceLastAction : 0
    ...
    <>
      ...
      {hourDiffSinceLastAction ? (
        <>
          ...
          <Text bold>{t('CHAM per hour: %amount%', { amount: earnedChamPerHour.toFixed(2) })}</Text>
