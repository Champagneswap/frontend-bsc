...
import { useCham, useSousChef, useVaultPoolContract } from 'hooks/useContract'
...
export const useVaultApprove = (vaultKey: VaultKey, setLastUpdated: () => void) => {
  ...
  const handleApprove = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(chamContract, 'approve', [vaultPoolContract.address, MaxUint256])
    })
    if (receipt?.status) {
      toastSuccess(
        t('Contract Enabled'),
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('You can now stake in the %symbol% vault!', { symbol: 'CHAM' })}
        </ToastDescriptionWithTx>,
      )
      setLastUpdated()
    }
...
export const useCheckVaultApprovalStatus = (vaultKey: VaultKey) => {
  ...
  const key = useMemo<UseSWRContractKey>(
    () =>
      account
        ? {
            contract: chamContract,
            methodName: 'allowance',
            params: [account, vaultPoolContract.address],
          }
        : null,
    [account, chamContract, vaultPoolContract.address],
  )
