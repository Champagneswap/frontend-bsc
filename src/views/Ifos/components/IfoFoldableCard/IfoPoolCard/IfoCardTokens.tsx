...
import { Token } from '@champagneswap/sdk'
import { Ifo, PoolIds } from 'config/constants/types'
import tokens from 'config/constants/tokens'
import { chamBnbLpToken } from 'config/constants/ifo'
...
const CommitTokenSection: React.FC<TokenSectionProps & { commitToken: Token }> = ({ commitToken, ...props }) => {
  if (commitToken.equals(chamBnbLpToken)) {
    return <TokenSection primaryToken={tokens.cham} secondaryToken={tokens.wbnb} {...props} />
  }
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough CHAM to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your CHAM.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate ChampagneSwap Profile to take part in next IFO‘s!')
                : t('You need an active ChampagneSwap Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (ifo.version === 3 && getBalanceNumber(credit) === 0) {
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average CHAM balance available to commit in the IFO CHAM pool.')}
            </MessageText>{' '}
      ...
      return userPoolCharacteristics.amountTokenCommittedInLP.isEqualTo(0) ? (
        <Flex flexDirection="column" alignItems="center">
          ...
          <Text textAlign="center" fontSize="14px">
            {t('To participate in the next IFO, stake some CHAM in the IFO CHAM pool!')}
          </Text>
