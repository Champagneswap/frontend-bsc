...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@champagneswap/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCham = ifo.currency === tokens.cham
  ...
  const tooltipContent =
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max CHAM entry is capped by minimum between your average CHAM balance in the IFO CHAM pool, or the pool’s hard cap. To increase the max entry, Stake more CHAM into the IFO CHAM pool',
        )
      : t(
          'For the unlimited sale, Max CHAM entry is capped by your average CHAM balance in the IFO CHAM pool. To increase the max entry, Stake more CHAM into the IFO CHAM pool',
        )

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCham ? t('Max. CHAM entry') : t('Max. token entry')
        ...
        value={
          <Text small textAlign="right" color={maxToken > 0 ? 'text' : 'failure'}>
            {`${formatNumber(maxToken, 3, 3)} ${
              !isCurrencyCham ? ifo.currency.symbol : ''
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].chamToBurn !== '$0' && <FooterEntry label={t('CHAM to burn:')} value={ifo[poolId].chamToBurn} />}
          <FooterEntry
