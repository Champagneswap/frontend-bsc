...
import { Flex } from '@champagneswap/uikit'
...
import { champagneBunniesAddress } from '../../../constants'
...
const IndividualPancakeBunnyPage = (props: IndividualPancakeBunnyPageProps) => {
  const collection = useGetCollection(champagneBunniesAddress)

  useFetchCollection(champagneBunniesAddress)
...
const IndividualPancakeBunnyPageBase: React.FC<IndividualPancakeBunnyPageProps> = ({ bunnyId }) => {
  ...
  useEffect(() => {
    const fetchBasicBunnyData = async () => {
      const { data } = await getNftsFromCollectionApi(champagneBunniesAddress)
      setNothingForSaleBunny({
        ...
        collectionAddress: champagneBunniesAddress,
    ...
    <Page>
      ...
      <TwoColumnsContainer flexDirection={['column', 'column', 'row']}>
        <Flex flexDirection="column" width="100%">
          ...
          <DetailsCard
            contractAddress={champagneBunniesAddress}
