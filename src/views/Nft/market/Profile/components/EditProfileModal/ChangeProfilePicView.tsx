...
import { Button, Box, InjectedModalProps, Text } from '@champagneswap/uikit'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onApprove: () => {
        ...
        return callWithGasPrice(contract, 'approve', [getChampagneProfileAddress(), selectedNft.tokenId])
      },
      onConfirm: () => {
        if (!profile.isActive) {
    ...
    <>
      ...
      {nfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a Champagne Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
