...
export const fetchActivityNftMetadata = async (activities: Activity[]): Promise<NftToken[]> => {
  const hasPBCollections = activities.some(
    (activity) => activity.nft.collection.id.toLowerCase() === champagneBunniesAddress.toLowerCase(),
  )
  let bunniesMetadata
  if (hasPBCollections) {
    bunniesMetadata = await getNftsFromCollectionApi(champagneBunniesAddress)
  }

  const pbNfts = bunniesMetadata
    ? activities
        .filter((activity) => activity.nft.collection.id.toLowerCase() === champagneBunniesAddress.toLowerCase())
        .map((activity) => {
